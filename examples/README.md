...
覆盖点与源码的一致性由 `tests/text/test_example_manifests.py` 保证，两端校验都在那里：声明的
覆盖点必须真在源码里、三篇的并集等于全部词表。多数覆盖点由源码里的固定标志判定，`asset_pdf`
这类描述附属文件构成的由 MANIFEST 字段判定，`nested_env` 这类语义性的没有低成本机械判据，
只校验它在词表内、仍靠人工维护。改动论文源码时同步维护 MANIFEST，测试会拦下脱节。
